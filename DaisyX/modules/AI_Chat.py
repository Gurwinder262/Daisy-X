...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Prabu AI Already Activated In This Chat")
            return
        await lel.edit(
            f"Prabu AI Successfully Added For Users In The Chat {message.chat.id}"
        )
        ...
        if not Escobar:
            await lel.edit("Prabu AI Was Not Activated In This Chat")
            return
        await lel.edit(
            f"Prabu AI Successfully Deactivated For Users In The Chat {message.chat.id}"
        )
...
async def inuka(client, message):
    ...
    test = test.replace("prabu", "Aco")
    test = test.replace("Prabu", "Aco")

    response = await lunaQuery(test, message.from_user.id if message.from_user else 0)
    response = response.replace("Aco", "Prabu")
    response = response.replace("aco", "Prabu")
...
@daisyx.on_message(
    filters.regex("Prabu|prabu|PrabuX|prabux|Prabux")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
...
__help__ = """
<b> Chatbot </b>
PRABU AI 3.0 IS THE ONLY AI SYSTEM WHICH CAN DETECT & REPLY UPTO 200 LANGUAGES
